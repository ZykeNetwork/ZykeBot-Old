...
exports.run = async (client, message, args, level) => { // eslint-disable-line no-unused-vars
  if (args.length !== 0) {
    ...
    if (/^[\w\ ]+$/i.test(line1) === false || /^[\w\ ]+$/i.test(line2) === false || /^[\w\ ]+$/i.test(line3) === false) return message.reply("only alphanumeric characters, spaces, and underscores are allowed!"); // eslint-disable-line no-useless-escape
...
exports.help = {
  name: "retro",
  category: "Image",
  description: "Reply with Retro Image",
  usage: "retro [text]"
};
